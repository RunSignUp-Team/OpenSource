<?php

// Server info
define('ENDPOINT', 'runsignup.com/Rest/');
define('PROTOCOL', 'https');

// User info
define('API_LOGIN_EMAIL', 'example@example.com');
define('API_LOGIN_PASSWORD', 'password');

// resultsApiTest1.php Settings
define('RESULTS_TEST1_RACE_ID', 123);
define('RESULTS_TEST1_EVENT_ID', 123);
define('RESULTS_TEST1_RESULT_SET_ID', 123);
define('RESULTS_TEST1_SETUP_NEW_DIVISIONS', true);
define('RESULTS_TEST1_NUM_PARTICIPANTS', 123);
define('RESULTS_TEST1_BATCH_SIZE', 25);	// Don't go too large here
define('RESULTS_TEST1_SLEEP_TIME', 1);	// Sleep between calls

// resultsApiTest2.php Settings
define('RESULTS_TEST2_RACE_ID', 123);
define('RESULTS_TEST2_EVENT_ID', 123);
define('RESULTS_TEST2_RESULT_SET_ID', 123);
define('RESULTS_TEST2_BATCH_SIZE', 25);	// Don't go too large here
define('RESULTS_TEST2_SLEEP_TIME', 1);	// Sleep between calls

// resultsApiTest3.php Settings
define('RESULTS_TEST3_RACE_ID', 123);
define('RESULTS_TEST3_EVENT_ID', 123);
define('RESULTS_TEST3_RESULT_SET_ID', 123);

?>